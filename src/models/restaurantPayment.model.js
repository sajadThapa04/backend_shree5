...
const restaurantPaymentSchema = new Schema({
  ...
  // Reference to the restaurantBooking related to the payment
  restaurantBooking: {
    type: Schema.Types.ObjectId,
...
restaurantPaymentSchema.index({user: 1});
restaurantPaymentSchema.index({restaurantBooking: 1});
restaurantPaymentSchema.index({paymentStatus: 1});
restaurantPaymentSchema.index({paymentMethod: 1});

// Auto-set refund status if the payment is refunded
restaurantPaymentSchema.pre("save", function (next) {
...
restaurantPaymentSchema.virtual("isPaymentSuccessful").get(function () {
  return this.paymentStatus === "paid";
});

// Method to process refunds
restaurantPaymentSchema.methods.processRefund = async function (refundAmount) {
...
restaurantPaymentSchema.plugin(mongoosePaginate);

const RestaurantPayment = mongoose.model("RestaurantPayment", restaurantPaymentSchema);
